...
# ==========================================
# 3. 持仓数据初始化 (已更新 7266 持仓)
# ==========================================
DEFAULT_CONFIG = {
    "c124": 1174300,  # 513100.SS
    "c216": 21600,    # 513300.SS
    "c320": 600,      # 2834.HK
    "c274": 77600,    # 7266.HK (已更新)
    "c_tqqq": 0,      # TQQQ
    "c_cash": 4550000.0
}
...
with st.sidebar:
    ...
    if st.button("💾 保存当前持仓", use_container_width=True, type="primary"):
...
# 获取实时数据
live_fx = get_live_rates()
live_prices = fetch_stock_data(holdings_map)
fx_final = {"USD": live_fx["USD"], "HKD": live_fx["HKD"], "CNY": 1.0}
...
# ==========================================
st.title("🛡️ 纳指平衡监控终端")
st.info(f"实时汇率已更新 (腾讯财经): **USD/CNY: {fx_final['USD']:.4f}** | **HKD/CNY: {fx_final['HKD']:.4f}**")
...
for t, h in holdings_map.items():
    ...
    df_list.append({
        ...
        "占比": f"{(v_cny/total_assets*100):.2f}%", "杠杆倍数": f"{h['lev']}x"
    })
st.table(pd.DataFrame(df_list))

# 第四行：调仓助手
st.markdown("---")
st.subheader("🎯 自动调仓模拟器")
